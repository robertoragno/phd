...
$$
logit(\bar{p}_{i}) = \alpha_{[ChrID]} + \beta_{[ChrID]} Alt_{i}
$$
...
Capr_Alt_fit.reth <- ulam(
    alist(
        ...
        transpars> theta <<- phi + 2.0,
        phi ~ dexp(1)
    ), data=dat.Caprine_Altitude , chains=4 )
...
### Test: Community by elevation

```{r}
#| echo: false

# Roman period
TestWMamm.R_lower_bound.a <- quantile(WMammals_Alt_fit.reth.post$a[,4], 0.25)
TestWMamm.R_upper_bound.a <- quantile(WMammals_Alt_fit.reth.post$a[,4], 0.75)

TestWMamm.R.a <- as.data.frame(WMammals_Alt_fit.reth.post) %>% 
  mutate(in_credible_range = (a.4 >= TestWMamm.R_lower_bound.a & a.4 <= TestWMamm.R_upper_bound.a))

TestWMamm.R_in_credible_range <- TestWMamm.R.a %>% 
  filter(in_credible_range == TRUE)

# pigs
TestPigs.R_lower_bound.a <- quantile(Pigs_Alt_fit.reth.post$a[,4], 0.25)
TestPigs.R_upper_bound.a <- quantile(Pigs_Alt_fit.reth.post$a[,4], 0.75)

TestPigs.R.a <- as.data.frame(Pigs_Alt_fit.reth.post) %>% 
  mutate(in_credible_range = (a.4 >= TestPigs.R_lower_bound.a & a.4 <= TestPigs.R_upper_bound.a))

TestPigs.R_in_credible_range <- TestPigs.R.a %>% 
  filter(in_credible_range == TRUE)

# cattle
TestCattle.R_lower_bound.a <- quantile(Cattle_Alt_fit.reth.post$a[,4], 0.25)
TestCattle.R_upper_bound.a <- quantile(Cattle_Alt_fit.reth.post$a[,4], 0.75)

TestCattle.R.a <- as.data.frame(Cattle_Alt_fit.reth.post) %>% 
  mutate(in_credible_range = (a.4 >= TestCattle.R_lower_bound.a & a.4 <= TestCattle.R_upper_bound.a))

TestCattle.R_in_credible_range <- TestCattle.R.a %>% 
  filter(in_credible_range == TRUE)

# caprine

TestCaprine.R_lower_bound.a <- quantile(Capr_Alt_fit.reth.post$a[,4], 0.25)
TestCaprine.R_upper_bound.a <- quantile(Capr_Alt_fit.reth.post$a[,4], 0.75)

TestCaprine.R.a <- as.data.frame(Capr_Alt_fit.reth.post) %>% 
  mutate(in_credible_range = (a.4 >= TestCaprine.R_lower_bound.a & a.4 <= TestCaprine.R_upper_bound.a))

TestCaprine.R_in_credible_range <- TestCaprine.R.a %>% 
  filter(in_credible_range == TRUE)


TestBinded <- bind_rows(
  TestCaprine.R.a %>% mutate(animal = "Caprine"),
  TestPigs.R.a %>% mutate(animal = "Pigs"),
  TestWMamm.R.a %>% mutate(animal = "WMamm"),
  TestCattle.R.a %>% mutate(animal = "Cattle")
)

ggplot(TestBinded, aes(x = a.4, y = b.4, color = animal)) +
  geom_point(alpha = .4) +
  theme_tidybayes() +
  ...
  labs(
    ...
    subtitle = "Altitude - Roman period",
    caption = "MCMC estimates kept in the logit scale. \nNegative slopes indicate a negative relationship between animal remains and increased altitude."
  )
