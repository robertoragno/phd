...
pigs_glm_mod <- brm(as.integer(Pigs) ~ Type*Chronology, # + (1 | ID)
                    data = Animals_Df, 
                    family=binomial(),
                    iter=3000,
                    warmup = 1000,
...
Pred <- predict(pigs_glm_mod, type = "response")
Pred <- if_else(Pred[,1] > 0.5, 1, 0)
ConfusionMatrix <- table(Pred, pull(Animals_Df, Pigs)) #`pull` results in a vector
