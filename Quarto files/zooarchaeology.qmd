...
# Modelling caprine against altitude
...
plot(
  subset(caprine_altitude, Chronology=="R")$Capr_NISP / 
    subset(caprine_altitude, Chronology=="R")$Tot_NISP
)
...
m12.1 <- ulam(
    alist(
        ...
        logit(pbar) <- a[Chr_id] + b[Chr_id]*log(Alt),
...
traceplot(Capr_Alt_fit, pars = c("a", "b"))

Capr_Alt_fit.post <- extract.samples(Capr_Alt_fit)


mean(Capr_Alt_fit.post$a[,1])
mean(Capr_Alt_fit.post$b[,1])

Alt.seq <- seq( 
    from = min(dat.Caprine_Altitude$Alt),
    to = max(dat.Caprine_Altitude$Alt),
    length.out=30
    )

Caprine_Alt_R.post <- sapply(Alt.seq, function(Altitude)
  inv_logit(Capr_Alt_fit.post$a[,1] +Capr_Alt_fit.post$b[,1] *log(Altitude))
    )

Caprine_Alt_R.PI <- apply(Caprine_Alt_R.post, 2, PI, prob=0.89)


plot(
  apply(Caprine_Alt_R.post, 2, mean) ~ (Alt.seq)
)

mcmc_nuts_energy(nuts_params(Capr_Alt_fit)) + ggtitle("NUTS Energy Diagnostic")
...
as.data.frame(dat.Caprine_Altitude) %>%
  group_by(Chr_id) %>%
  mutate(Chronology = case_when(
    Chr_id == 1 ~ "EMA",
    Chr_id == 2 ~ "LR",
  ...
  data_grid(Alt = seq_range(log(Alt), n = 101), Chronology) %>%
  ...
  theme(legend.position = "bottom", 
    ...
    labs(
       ...
       title="Caprine",
       subtitle="Altitude",
       caption = expression(paste("Precision parameter (", phi, "): 8.13"))
       )+
