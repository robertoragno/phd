...
curve( dbeta2(x,pbar,theta) , from=0 , to=1 ,
    xlab="Probability" , ylab="Density", main="phi < 2" )
...
To estimate the animals' occurrences probability in each century, I used a betabinomial distribution to model overdispersion in the data. The $A$ on the left side of the formula is the outcome variable---the animal NISP counts for each observation $i$. This is an intercept-only model, where the intercept $\alpha$ carries an index ${[Century]}$ as the model will provide estimates for each century under examination. The $\phi$ parameter indicates the precision in the Beta distribution, modelled by century.
...
$$
\phi_{[Century]} \sim Exponential(1)
$$
...
Animals_by_Century_a.plot <- Animals_by_Century_a_fit %>%
  ...
  labs(
    ...
    title = "Animals",
    subtitle = "Century",
    caption = "The dashed line indicates the diachronical average of occurrence for each animal. The circle indicates the mean for each line."
  ) +
  theme_tidybayes() +
  theme(
...
## Trends by traditional chronologies

To estimate the animals' occurrences probability in each chronology, I used a betabinomial distribution to model overdispersion in the data. The $A$ on the left side of the formula is the outcome variable---the animal NISP counts for each observation $i$. This is an intercept-only model, where the intercept $\alpha$ carries an index ${[ChrID]}$ as the model will provide estimates for each chronology under examination. The $\phi$ parameter indicates the precision in the Beta distribution, modelled by chronology.
...
ggplot(Animals_by_SiteType_fit_comm, aes(x = inv_logit(a), y = Precis, color = Type)) +
  geom_point(alpha = .4) +
  theme_tidybayes() +
  scale_color_brewer(palette = "Spectral") +
  theme(
  ...
  facet_grid(factor(Chronology, levels=chrono_levels)~animal, scales="free")
