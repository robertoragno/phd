...
# Why does it think that Urban:ChronologyMa is NA? 


AIC(pigs_beta_mod1)

round(pigs_beta_mod1@random.param, 3)    # from the beta - binomial model

Mod_Levels <- expand.grid(Type = levels(Animals_Df$Type),
                     Chronology = levels(Animals_Df$Chronology))
...
#| echo: false
#| eval: false
caprine_beta_mod1 <- aod::betabin(
  cbind(Caprine, Tot_NISP - Caprine) ~ Chronology*Geo, ~ 1, 
  data = Animals_Df,
  control = list(maxit = 10000)
                               )

Mod_Levels_Cap <- expand.grid(Geo = levels(Animals_Df$Geo),
                     Chronology = levels(Animals_Df$Chronology))

New.pred.Capr <- data.frame(Mod_Levels_Cap, predict(caprine_beta_mod1, Mod_Levels_Cap, se = TRUE, type = "response"))
  
Caprine_Pred_Plot <-ggplot(New.pred.Capr) +
  geom_bar(aes(x=Chronology, 
               y=fit, 
               fill=factor(Chronology, 
                           levels=(level_order))), 
  ...
  theme_pubclean()+
  labs(
    title="Fitted Beta-Binomial GLM: Caprine",
    subtitle = "Barplot with associated error bars.",
    y="Fitted values",
    fill=""
  )+
