...
Mapping the use of minor grains by century in each of the three macroregions. Their diffusion is not linear as shown by the cdplots. There are two options here probably:

-   using just the presence/absence of minor grains.

    -   Problem 1: Include Emmer? It is a minor grain, but it is still the only minor grain used in southern Italy.

    -   Problem 2: It is not very indicative of how much minor grains are important.

-   counting how many minor grain taxa are found in each context.

    -   Problem 1: Not binomial anymore, maybe poisson. Is it giving better results? Not really.

Another problem for both models is that probably melting the entries by century gives a better chance to entries where the chronology is more uncertain. For instance, if a context spans 5 centuries it will be repeated 5 times in this way.
...
#| eval: true
par(mfrow=c(1,3))
cdplot(as.factor(Present)~Chronology,data=subset(Minor_Grains, Macroregion=="Southern Italy"), main="Southern Italy", xlab="Century", ylab="Presence vs Absence of Minor Grains")
cdplot(as.factor(Present)~year,data=subset(Minor_Grains, Macroregion=="Central Italy"), main="Central Italy", xlab="Century", ylab="Presence vs Absence of Minor Grains")
cdplot(as.factor(Present)~year,data=subset(Minor_Grains, Macroregion=="Northern Italy"), main="Northern Italy", xlab="Century", ylab="Presence vs Absence of Minor Grains")

m_mg1 <- glm(Present~Macroregion*year, family = binomial(link="logit"), data=Minor_Grains)

summary(m_mg1)
cat("Pseudo R-squared: ", 1-(m_mg1$deviance/m_mg1$null.deviance))

SouthPred.data  <- data.frame(year=-1:12,Macroregion='Southern Italy')
NorthPred.data  <- data.frame(year=-1:12,Macroregion='Northern Italy')
CentralPred.data  <- data.frame(year=-1:12,Macroregion='Central Italy')

South.pred  <- predict(m_mg1,newdata = SouthPred.data,type = 'response')
North.pred  <- predict(m_mg1,newdata = NorthPred.data,type = 'response')
Central.pred  <- predict(m_mg1,newdata = CentralPred.data,type = 'response')

# Assign colors to each region
Minor_Grains$col='darkgreen'
Minor_Grains$col[which(Minor_Grains$Macroregion=='Southern Italy')]='lightblue'
Minor_Grains$col[which(Minor_Grains$Macroregion=='Central Italy')]='goldenrod'


# Make an empty plot
par(mfrow=c(1,1))
plot(NULL,xlim=c(-1,12),ylim=c(0,1),xlab='Century',ylab='Probability of Minor Grains')
# Add observed data:
points(Minor_Grains$year,jitter(Minor_Grains$Present,factor = 0.1),col=Minor_Grains$col)
# Add model predictions
lines(-1:12,South.pred,col='lightblue',lwd=2)
lines(-1:12,North.pred,col='darkgreen',lwd=2)
lines(-1:12,Central.pred,col='goldenrod',lwd=2)

# Add legend
legend('topright', title="Macroregion", legend=c('Southern','Northern', 'Central'),fill=c('lightblue','darkgreen','goldenrod'))

```

#### Option 2

The pseudo-$R^2$ is slightly better, but the residual deviance is much bigger than the degrees of freedom.

```{r}
#| echo: false
#| warning: false
#| message: false
m_mg2 <- glm(Grain_Count~Macroregion*year, family = poisson, data=Minor_Grains)

summary(m_mg2)
cat("Pseudo R-squared: ", 1-(m_mg2$deviance/m_mg2$null.deviance))
```
