...
### Ubiquity - Jeffreys interval

Plot only the most important plants probably
...
colors = gray(seq(0.7, 0.15, length.out = 12))
```
...
test2 <- data.frame(
  ...
  shape1=plant_tables[["Vitis.vinifera"]]$Beta1[5],
  shape2=plant_tables[["Vitis.vinifera"]]$Beta2[5]
))

plot(density(test2$Distribution))

ggplot(test2, aes(y=Distribution)) +
...
$$ D_{i} \sim Beta(\mu_{i}, \phi_{i}) $$ 
$$logit(\mu_{i}) = a_{[ChrID]}$$ 
$$a_{[ChrID]} \sim Normal(0,1.5)$$ 
$$\phi_{i} \sim Exp(0.1)+2$$
