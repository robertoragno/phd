...
The richness of sites across the chronologies was calculated . For the calculation, only sites that provided cereals, legumes, fruits and nuts have been used. Although this choice entails the loss of many observations, it was necessary in order to produce credible results. The graph shows extended credible intervals for groups with fewer observations. The mean plants richness has been calculated using a Poisson distribution. The model uses each site ${i}$ richness ($R_{i}$) as the outcome variable, calculated as follows

$$ R_{i} = \sum_{n=1}^{k} Species_{ni}$$
where $k$ is the total number of species in a site. The goal is then to model the mean richness $\lambda_{i}$. In addition to modelling four intercepts, one for each chronology $ChrID$, the regression also includes a varying slope for each observation. This will later be informative in quantifying on average how much the observations in each chronology deviate from the mean (intercept). The formula for the Poisson regression is as follows, where the priors for the intercept and slope have been chosen to be weakly informative, so that the range of the possible $\lambda_{i}$ will remain in the outcome space.
...
$$
log {\lambda}_{i} = Intercept_{[ChrID]} + Slope_{[Observations]}
$$

$$
Intercept_{[ChrID]} \sim Normal(3,0.5)
$$

$$
Slope_{[Observations]} \sim Normal(0,0.2)
$$
...
plot(density(exp(rnorm(1e4,3,0.5)+rnorm(1e4,0,0.2))), main="Prior Predictive Simulation", col=rangi2)
```
...
Plants_Richness_by_Chronology_fit <- 
  ulam(
    alist(
        Richness ~ dpois( p ),
        log(p) <- Intercept[Chronology] + Slope[Observations],
        Intercept[Chronology] ~ dnorm(3,0.5),
        Slope[Observations] ~ dnorm(0,0.2)
    ), data=dat.Plants_Richness , chains=4 , log_lik=TRUE )

Plants_Diversity_Shannon_by_Chronology_fit <- 
  ulam(
   alist(
        Diversity_Shannon_Eq ~ dnorm( mu , sigma ),
        mu <- Intercept[Chronology],
        Intercept[Chronology] ~ dnorm(0,1.5),
        sigma ~ dexp(1)
...
Plants_Diversity_Simpson_by_Chronology_fit <- 
  ulam(
   alist(
        Diversity_Simpson ~ dnorm( mu , sigma ),
        mu <- Intercept[Chronology],
        Intercept[Chronology] ~ dnorm(0,1.5),
        sigma ~ dexp(1)
    ), data=dat.Plants_Diversity , chains=4, iter = 4000
...
Plants_Richness_by_Chronology_fit_Intercept <- Plants_Richness_by_Chronology_fit %>%
  spread_draws(Intercept[Chronology]) %>% 
...
Plants_Richness_by_Chronology_fit_Slope <- Plants_Richness_by_Chronology_fit %>%
  spread_draws(Slope[Observations])

Plants_Richness_by_Chronology_fit_Slope_Means <- aggregate(Plants_Richness_by_Chronology_fit_Slope$Slope, 
                   by = list(Plants_Richness_by_Chronology_fit_Slope$Observations), 
                   FUN = mean)

Plants_Richness_by_Chronology_fit_Slope_Means$x <- exp(Plants_Richness_by_Chronology_fit_Slope_Means$x)

Plants_Richness_by_Chronology_fit_Slope_Means$Chronology <- Plants_Diversity_for_Richness$Chronology

colnames(Plants_Richness_by_Chronology_fit_Slope_Means) <- c("Observation", "Slope", "Chronology")

Plants_Diversity_Shannon_by_Chronology_fit_Intercept <- Plants_Diversity_Shannon_by_Chronology_fit %>%
  spread_draws(Intercept[Chronology]) %>% 
...
Plants_Diversity_Simpson_by_Chronology_fit_Intercept <- Plants_Diversity_Simpson_by_Chronology_fit %>%
  spread_draws(Intercept[Chronology]) %>% 
...
Plants_Diversity_Indices_by_Chronology_Means <- data.frame(
  ...
  means=c(
   mean(Plants_Diversity_Shannon_by_Chronology_fit_Intercept$Intercept),
   mean(Plants_Diversity_Simpson_by_Chronology_fit_Intercept$Intercept)
    )
)

Plants_Diversity_Indices_by_Chronology_fit_Intercept <- bind_rows(Plants_Diversity_Shannon_by_Chronology_fit_Intercept,Plants_Diversity_Simpson_by_Chronology_fit_Intercept)
...
Plants_Richness_by_Chronology_fit_Intercept %>%
  ggplot(aes(
  ...
  stat_summary(
    ...
    geom_vline(
    aes(xintercept = exp(mean( Plants_Richness_by_Chronology_fit_Intercept$Intercept))),
    linetype = "twodash",
    linewidth = 0.45,
    color = "red4"
  ) +
...
Plants_Richness_by_Chronology_fit_Slope_Means %>%
  ggplot(aes(
    y = factor(Chronology, levels = chrono_levels),
    x = Slope
  )) +
  ...
  scale_color_brewer(palette = "Greys", name = "Credible interval") +
  labs(
    x = "Slope (deviation from the mean)",
    y = "Chronology",
    title = "Plant Richness - Varying effect",
    caption = "Predicted mean sites deviation from the intercept, plotted by chronology."
  ) +
  theme_tidybayes() +
  theme(
    legend.position = "bottom",
...
The diversity of sites has been calculated using the package `vegan` and the Shannon equitability (H') and Simpson indices, both ranging from 0 (for complete unevenness) to 1 (maximum diversity). Only complete cases have been used in the calculations as for the sites richness. The graph shows extended credible intervals for groups with fewer observations. In particular, the Shannon equitability index showed higher variability in the 11th century. Consequently, its credible range is very wide.
```{r}
#| echo: false


Plants_Diversity_Indices_by_Chronology_fit_Intercept %>%
  ggplot(aes(
    y = factor(Chronology, levels = chrono_levels),
    x = Intercept
  )) +
...
#| fig-cap: "Prior predictive simulation for the binomial models used in this section."
...
plot(
  density(inv_logit(
    ...
    ), main="Prior Predictive Simulation", sub="inv_logit(Normal(0,1.5)"
  )
```
...
plot(
  density(inv_logit(
    ...
    ), main="Prior Predictive Simulation", sub="inv_logit(Normal(0,1.5)+Normal(0,1.5)*Altitude(km))"
  )
```
...
Cereals_Alt_post.plot <- ggplot(Cereals_Alt_post, aes(x = a, y = b, color = Plant)) +
  geom_point(alpha = .3) +
  theme_tidybayes() +
...
plot(
  density(inv_logit(
    rnorm(1e4, 0, 1.3)+rnorm(1e4, 0, 1.3)*sample(bot_altitude_precip$Precip_Mean, size=1e4, replace=T)
    )
    ), main="Prior Predictive Simulation: One coefficient slope models", sub="inv_logit(Normal(0,1.3)+Normal(0,1.3)*Mean_Precipitation (std))"
  )
...
plot(
  density(
    ...
    ), main="Prior Predictive Simulation: Two coefficients slope model", sub="inv_logit(Normal(0,1.3)+Normal(0,1.5)*Mean_Precipitation (std)+Normal(0,1.3)*Altitude (km)"
  )
...
plot(
  density(inv_logit(
    rnorm(1e4, 0, 1.3)+rnorm(1e4, 0, 1.3)*sample(bot_altitude_temperature$temperature_Mean, size=1e4, replace=T)
    )
    ), main="Prior Predictive Simulation: One coefficient slope models", sub="inv_logit(Normal(0,1.3)+Normal(0,1.3)*MeanAnnualTemperature (std))"
  )
...
plot(
  density(
    inv_logit(
    rnorm(1e4, 0, 1.3) + 
    rnorm(1e4, 0, 1.3)*sample(bot_altitude_temperature$temperature_Mean, 
                              size=1e4, replace=T) +
    rnorm(1e4, 0, 1.3)*sample(bot_altitude_precip$site_altitude/1000, 
                              size=1e4, replace=T)
    )
    ), main="Prior Predictive Simulation: Two coefficients slope model", sub="inv_logit(Normal(0,1.3)+Normal(0,1.3)*MeanAnnualTemperature (std)+Normal(0,1.3)*Altitude (km)"
  )
