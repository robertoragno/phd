...
m_mg_reg.plot <- m_mg_reg %>%
  spread_draws(RegChr[RC_ID]) %>%
  ggplot(aes(y = fct_rev(
    factor(m_mg_reg.df$Chrono, 
           levels=c("R", "LR", "EMA", "Ma"))
    ), 
    x = inv_logit(RegChr))) +
  stat_interval(
    .width = c(.50, .80, .95, .99), 
  ...
  facet_grid(~factor(m_mg_reg.df$Region, levels=c("SI", "CI", "NI")))
...
#| fig-cap: "Bayesian probability estimates for minor grains (Emmer, Einkorn, Foxtail and Broomcorn millets, Sorghum), divided by macroregion and chronology. The prediction estimates are produced with a GLM and weakly informative priors. Credibility intervals are provided."
...
m_ng_type.plot <- ggplot(data=m_ng_type.df, 
       aes(
         x=factor(Chrono, levels=c("R", "LR", "EMA", "Ma")),
         y=inv_logit(TypeChr), 
         ), 
       ) +
...
m_mg_type.plot <- ggplot(data=m_mg_type.df, 
       aes(
         x=factor(Chrono, levels=c("R", "LR", "EMA", "Ma")),
         y=inv_logit(TypeChr), 
         ), 
...
m_leg_type.plot <- ggplot(data=m_leg_type.df, 
       aes(
         x=factor(Chrono, levels=c("R", "LR", "EMA", "Ma")),
         y=inv_logit(TypeChr), 
         ), 
...
m_grape_type.plot <- ggplot(data=m_grape_type.df, 
       aes(
         x=factor(Chrono, levels=c("R", "LR", "EMA", "Ma")),
         y=inv_logit(TypeChr), 
         ), 
...
m_olives_type.plot <- ggplot(data=m_olives_type.df, 
       aes(
         x=factor(Chrono, levels=c("R", "LR", "EMA", "Ma")),
         y=inv_logit(TypeChr), 
         ), 
...
m_nuts_type.plot <- ggplot(data=m_nuts_type.df, 
       aes(
         x=factor(Chrono, levels=c("R", "LR", "EMA", "Ma")),
         y=inv_logit(TypeChr), 
         ), 
...
m_dfruit_type.plot <- ggplot(data=m_dfruit_type.df, 
       aes(
         x=factor(Chrono, levels=c("R", "LR", "EMA", "Ma")),
         y=inv_logit(TypeChr), 
         ), 
