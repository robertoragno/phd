...
$$log {\lambda}_{i} = \alpha_{[ChrID]} + \beta_{[Observations]}$$
$$\alpha_{[ChrID]} \sim Normal(3,0.5)$$
$$\beta_{[Observations]} \sim Normal(0,0.2)$$
...
Plants_Diversity_Shannon_by_Chronology_fit <- 
  ulam(
   alist(
        ...
        sigma ~ dexp(1)
    ), data=dat.Plants_Diversity , chains=4, iter = 4000, log_lik = T
    )

# TESTS 
Plants_Diversity_Shannon_by_Chronology_fit_BETA <- 
  quap(
   alist(
        Diversity_Shannon_Eq ~ dbeta2( mu , phi ),
        mu <- a[Chronology],
        a[Chronology] ~ dunif(0,1),
        phi ~ dexp(1)
    ), data=dat.Plants_Diversity
    )
