...
The variation to the formula that I would like to apply is the following, where the dispersion $\theta$ varies accordingly to the `TC_id` interaction parameter. I am using the same parameter of the $\alpha$ intercept because I want the dispersion for each intercept ( = I want them in the same for cycle). 

$$P_{i} \sim BetaBinomial(NISP_{i}, \bar{p}_{i} , \theta )$$
...
Judging from the STAN output of the previous model (`m12.1`), it looks like $\theta$ is already varying according to the index `TC_id` (as it is iteratively multiplied by pbar), although I am not getting what I need: phi[1], phi[2], etc. 
...
# Function to create plot for the distribution - NOT WORKING ANYMORE
